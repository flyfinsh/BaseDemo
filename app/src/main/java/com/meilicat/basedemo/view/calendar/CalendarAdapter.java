...
//			drawable = res.getDrawable(R.drawable.calendar_item_selected_bg);
		...
		//以不同的日期，去存储数据
		if (mStartPosition <= position + 7 && position <= mEndPosition - 7){
			//就存储 成功了。。。
			mViews.put(dayNumber[position].split("\\.")[0], textView);
//			Log.i("hero", dayNumber[position].split("\\.")[0] + "-------" + textView+"------"+mViews.size());
			if (position == currentFlag){
				mCurrentView = textView;
//				Log.i("hero","给当前的view赋值------"+mCurrentView);
			}
		}
	...
	private Map<String,View> mViews = new HashMap<String, View>();
	//定义一个map集合，用于保存，当天的view的实例
...
//		Log.d("DAY", isLeapyear + " ======  " + daysOfMonth + "  ============  " + dayOfWeek + "  =========   " + lastDaysOfMonth);
		...
		mStartPosition = dayOfWeek+7;
		mEndPosition = dayOfWeek+daysOfMonth+7-1;
	}
	...
	public void changeItemColor(int position){
		String s = dayNumber[position].split("\\.")[0];
		View view = mViews.get(s);
		//重新绘制一遍数据


		if (preView != null){
//			Log.i("hero",(mCurrentView != null)+"---"+preView+"-----"+mCurrentView);
			if (mCurrentView != null && preView == mCurrentView){
				//说明上一个view是当天的日期
...
//				LogUtils.i(position + "");
				...
				if (isWeekday){
					preView.setTextColor(Color.rgb(23, 126, 214));
				}else{
					preView.setTextColor(Color.BLACK);
				}
		...
		//设置选中的view的颜色
		if (view != null){
			TextView byId = (TextView) view.findViewById(R.id.tvtext);

			//得到view，其实就是一个TextView，然后设置颜色
			byId.setTextColor(Color.WHITE);
			byId.setBackgroundColor(Color.RED);
			preView = byId;
		}
	...
	private boolean isWeekday = false;//默认不是周末
