...
import com.meilicat.basedemo.view.CropImageView;
 ...
 * 
 */
public class ClipPictureActivity extends BaseTitleActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            ...
            if (TextUtils.isEmpty(mImagePath)) {
                ViewUtils.showToast(getApplicationContext(), "没有选择图片");
                return;
            }
        ...
        if (mBitmap == null) {
            Log.d(TAG, "finish!!!");
            ViewUtils.showToast(getApplicationContext(), "图片加载失败，不能进行图片裁剪");
            finish();
            return;
        }
